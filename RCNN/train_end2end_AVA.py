...
def clean_up(train_ins, err_msg=""):
    # AVA-SDK instance ends the done fuction is used for upload status and cleaning the workspace
    if train_ins == None:
        return
...
def start_new_training(args, ctx, pretrained, epoch, prefix, begin_epoch, end_epoch,lr=0.001, lr_step='5'):
    try:
        # binding the singal
        ...
        # AVA-SDK initalize the Instance
        ...
        train_net(args, ctx, pretrained, epoch, prefix, begin_epoch, end_epoch, batch_end_cb,
...
def train_net(args, ctx, pretrained, epoch, prefix, begin_epoch, end_epoch,batch_end_callback,
              lr=0.001, lr_step='5'):
    # set up logger
    logging.basicConfig()
    ...
    if os.path.exists(args.ava_roidb_name):
        with open(args.ava_roidb_name, 'rb') as fid:
            roidb = cPickle.load(fid)
    else:
        print("missing roidb " + args.ava_roidb_name)
        exit()
...
def parse_args():
    ...
    parser.add_argument('--ava_roidb_name', help='recordio name should in dataset_path',type=str)
...
def main():
    ...
    start_new_training(args, ctx, args.pretrained, args.pretrained_epoch, args.prefix, args.begin_epoch, args.end_epoch,
