...
def get_rpn_batch_from_recordio(IOitem, class_to_ind,use_data_augmentation = False):#50
    """
    prototype for rpn batch: data, im_info, gt_boxes
    :param roidb: ['image', 'flipped'] + ['gt_boxes', 'boxes', 'gt_classes']
    :return: data, label
    ...
    anno, img = jsonUnpack(IOitem[0]) # 9.2s
    import cv2
    import random
    import json
    im_array = cv2.imdecode(np.fromstring(img, dtype=np.uint8),-1) #23s
    ...
    im_array, im_scale = resize(im_array, target_size, max_size, stride=config.IMAGE_STRIDE)#7.5
    im_tensor = transform(im_array, config.PIXEL_MEANS)# 10s
