...
class AnchorLoaderAvaRecordIO(mx.io.DataIter):
    ...
    def get_batch(self): # 85s
        ...
        for islice in slices:
            iroidb = [roidb[i] for i in range(islice.start, islice.stop)]
            data, label = get_rpn_batch_from_recordio(iroidb, self.class_to_ind, self.use_data_augmentation) #50s
            data_list.append(data)
            label_list.append(label)

        # pad data first and then assign anchor (read label)
        data_tensor = tensor_vstack([batch['data'] for batch in data_list])  # 12s
        for data, data_pad in zip(data_list, data_tensor):
            data['data'] = data_pad[np.newaxis, :]
        ...
        for data, label in zip(data_list, label_list):
            ...
            label = assign_anchor(feat_shape, label['gt_boxes'], data['im_info'],
                                  self.feat_stride, self.anchor_scales,
                                  self.anchor_ratios, self.allowed_border) #8s
            new_label_list.append(label)
